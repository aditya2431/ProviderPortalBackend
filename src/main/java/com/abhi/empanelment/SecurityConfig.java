 ...
 * Disable X-Frame-Options for H2 Console .addHeaderWriter(new
 * StaticHeadersWriter("Content-Security-Policy", "script-src 'self'"))
 * .xssProtection().block(true)// Enable XSS Protection; //
 * .and().httpStrictTransportSecurity().disable();// Disable Strict Transport
 * Security (HSTS) for development .and().httpStrictTransportSecurity()
